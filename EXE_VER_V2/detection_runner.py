...
from pipeline_config import DetectionConfig, DetectionResult
...
def _validate_args(args: argparse.Namespace) -> str:
    ...
    if not args.local_model.exists():
        raise SystemExit(f"Local model not found: {args.local_model}")
    return ""
...
def _load_yolo_model(model_path: Path) -> Any:
    ...
    if is_onnx and importlib.util.find_spec("onnxruntime") is None:
        raise SystemExit(
            "Missing required package for ONNX local inference: onnxruntime\n"
...
def _is_rfdetr_model(model_path: Path) -> bool:
    metadata_paths = [
        model_path.parent / "model_type.json",
        model_path.parent / "environment.json",
        model_path.parent / "mode_type",
        model_path.parent / "mode_type.txt",
    ]
    for metadata_path in metadata_paths:
        if not metadata_path.exists() or not metadata_path.is_file():
            continue
        text = metadata_path.read_text(encoding="utf-8", errors="ignore").lower()
        if "rfdetr" in text or "rf-detr" in text:
            return True
    return False


def _load_rfdetr_model(model_path: Path) -> LocalRfdetrModel:
    try:
        import onnxruntime as ort
    except ImportError as error:
        raise SystemExit(
            "Missing required package for RF-DETR ONNX local inference: onnxruntime\n"
            "Install it first:\n  python3 -m pip install onnxruntime"
        ) from error
    ...
    if not inputs:
        raise SystemExit(f"ONNX model has no inputs: {model_path}")
    imgsz = _fixed_onnx_imgsz_from_session(session)
    if imgsz is None:
        raise SystemExit(f"Could not read fixed RF-DETR input size from: {model_path}")
    return LocalRfdetrModel(session=session, input_name=inputs[0].name, imgsz=imgsz)


def _fixed_onnx_imgsz_from_session(session: Any) -> int | tuple[int, int] | None:
    inputs = session.get_inputs()
    if not inputs:
        return None
...
def _fixed_onnx_imgsz(model_path: Path) -> int | tuple[int, int] | None:
    ...
    session = ort.InferenceSession(str(model_path), providers=["CPUExecutionProvider"])
    return _fixed_onnx_imgsz_from_session(session)


def _load_sidecar_class_names(model_path: Path) -> list[str]:
    candidates = [
        model_path.with_suffix(".names"),
        model_path.with_suffix(".labels"),
        model_path.parent / "class_name",
        model_path.parent / "class_names",
        model_path.parent / "class_name.txt",
        model_path.parent / "class_names.txt",
    ...
    for candidate in candidates:
        if not candidate.exists() or not candidate.is_file():
            continue
        names = _parse_class_names_file(candidate)
        if names:
            return names
    return []


def _parse_class_names_file(path: Path) -> list[str]:
    ...
    try:
        ...
        loaded = None
    names = _class_names_from_loaded_data(loaded)
    if names:
        return names
    ...
    if path.suffix.lower() in {".yaml", ".yml"}:
        ...
        if yaml is not None:
            ...
            names = _class_names_from_loaded_data(loaded_yaml)
            if names:
                return names
...
def _class_names_from_loaded_data(value: Any) -> list[str]:
    if isinstance(value, list):
        return [str(item) for item in value if str(item)]
    if isinstance(value, dict):
        names = value.get("names") or value.get("class_names") or value.get("class_name") or value.get("classes")
        ...
        if isinstance(names, str):
            return [part.strip() for part in names.replace(",", "\n").splitlines() if part.strip()]
    return []
...
def _to_args(config: DetectionConfig) -> argparse.Namespace:
    return argparse.Namespace(
        ...
        local_onnx_folder=Path(config.local_onnx_folder),
        local_pt_model=Path(config.local_pt_model),
...
def _yolo_class_name(model: Any, yolo_result: Any, class_id: int | None, class_names: list[str]) -> str:
    if class_id is None:
    ...
    for index, xyxy in enumerate(xyxy_values):
        ...
        detections.append(
            detection_core.Detection(
                class_name=_yolo_class_name(model, yolo_result, class_id, class_names),
...
def _imgsz_hw(imgsz: int | tuple[int, int]) -> tuple[int, int]:
    if isinstance(imgsz, tuple):
        return int(imgsz[0]), int(imgsz[1])
    return int(imgsz), int(imgsz)


def _rfdetr_input_tensor(tile_rgb: Any, image_cls: Any, imgsz: int | tuple[int, int]) -> Any:
    import numpy as np

    height, width = _imgsz_hw(imgsz)
    image = image_cls.fromarray(tile_rgb).resize((width, height))
...
def _is_background_class(class_id: int, class_names: list[str]) -> bool:
    if not 0 <= class_id < len(class_names):
        return False
    name = class_names[class_id].strip().lower()
    return name.startswith("background") or name in {"background", "no_object", "no-object", "__background__"}


def _rfdetr_output_arrays(outputs: list[Any], class_names: list[str]) -> tuple[Any, Any]:
    import numpy as np
    ...
    for array in arrays:
        if array.ndim == 3 and array.shape[-1] == 4:
            boxes = array
            break
    expected_class_count = len(class_names) if class_names else None
    for array in arrays:
        if array.ndim != 3 or array.shape[-1] == 4:
            continue
        if expected_class_count is None or array.shape[-1] == expected_class_count:
            logits = array
            break
    if logits is None:
        for array in arrays:
    ...
    if boxes is None or logits is None:
        ...
        raise SystemExit(f"Could not parse RF-DETR ONNX outputs. Output shapes: {shapes}")
    return boxes, logits
...
def _detections_from_rfdetr_outputs(
    outputs: list[Any],
    class_names: list[str],
    window: Any,
    image_width: int,
    ...
    boxes, logits = _rfdetr_output_arrays(outputs, class_names)
    boxes = boxes[0]
    scores = _sigmoid(logits[0])
    query_indexes, class_indexes = np.where(scores >= confidence_threshold)
    detections: list[detection_core.Detection] = []

    for query_index, class_id in zip(query_indexes.tolist(), class_indexes.tolist()):
        if _is_background_class(class_id, class_names):
            continue

        class_score = float(scores[query_index, class_id])
        cx, cy, width, height = [float(value) for value in boxes[query_index, :4]]
        ...
        xmin = max(0.0, min(float(image_width), float(window.col_off) + cx - width / 2.0))
        ymin = max(0.0, min(float(image_height), float(window.row_off) + cy - height / 2.0))
        xmax = max(0.0, min(float(image_width), float(window.col_off) + cx + width / 2.0))
        ymax = max(0.0, min(float(image_height), float(window.row_off) + cy + height / 2.0))
        if xmax <= xmin or ymax <= ymin:
            continue
        ...
        detections.append(
            detection_core.Detection(
                class_name=_yolo_class_name(None, None, class_id, class_names),
...
def _process_local_yolo_tile(
    ...
    try:
        with rasterio_module.open(image_path) as src:
            ...
            if args.local_model.suffix.lower() == ".onnx":
...
def _process_local_rfdetr_tile(
    index: int,
    window: Any,
    image_path: Path,
    temp_dir_path: Path,
    args: argparse.Namespace,
    model: LocalRfdetrModel,
    rasterio_module: Any,
    image_cls: Any,
    class_names: list[str],
) -> detection_core.TileResult:
    ...
    try:
        with rasterio_module.open(image_path) as src:
            tile_rgb = detection_core.build_tile_image(src, window, image_cls, tile_path, args.jpg_quality)
            input_tensor = _rfdetr_input_tensor(tile_rgb, image_cls, model.imgsz)
            outputs = model.session.run(None, {model.input_name: input_tensor})
            if index == 1:
                notice = f"RF-DETR ONNX output shapes: {', '.join(str(getattr(output, 'shape', '?')) for output in outputs)}"
            tile_detections = _detections_from_rfdetr_outputs(
                outputs=outputs,
                class_names=class_names,
                window=window,
                image_width=src.width,
                ...
                tile_width=tile_rgb.shape[1],
                tile_height=tile_rgb.shape[0],
            )
...
def run_detection(
    ...
    if args.inference_backend == "local_yolo":
        ...
        yolo_class_names = _load_sidecar_class_names(args.local_model)
        if _is_rfdetr_model(args.local_model):
            local_model = _load_rfdetr_model(args.local_model)
            local_model_kind = "rfdetr"
            yolo_imgsz = local_model.imgsz
        else:
            local_model = _load_yolo_model(args.local_model)
            local_model_kind = "yolo"
            yolo_imgsz = _fixed_onnx_imgsz(args.local_model)
    else:
    ...
    with rasterio.open(args.image) as src:
        ...
        if args.inference_backend == "local_yolo":
            _log(log, f"Local model: {args.local_model}")
            if local_model_kind == "rfdetr":
                _log(log, "Local model type: RF-DETR ONNX")
            if yolo_imgsz is not None:
                _log(log, f"Local ONNX input size: {yolo_imgsz}")
            if yolo_class_names:
                _log(log, f"Loaded local model class names: {', '.join(yolo_class_names)}")
            if args.workers > 1:
                _log(log, "Local model backend runs one in-process model; tile inference is processed sequentially.")
            ...
            if windows and not (cancel_event and cancel_event.is_set()):
                if args.inference_backend == "local_yolo":
                    if local_model is None:
                        raise SystemExit("Local model was not loaded.")
                    for index, window in enumerate(windows, start=1):
                        if cancel_event and cancel_event.is_set():
                            _log(log, "Cancellation requested. Stopping before next tile.")
                            break
                        if local_model_kind == "rfdetr":
                            result = _process_local_rfdetr_tile(
                                index=index,
                                window=window,
