...
from pipeline_config import (
    DEFAULT_LOCAL_MODEL,
    DEFAULT_LOCAL_ONNX_FOLDER,
    DEFAULT_LOCAL_PT_MODEL,
    DEFAULT_OUTPUT_ROOT,
    DEFAULT_ORTHOMOSAIC,
    PROJECT_ROOT,
    DetectionConfig,
...
from runtime_paths import APP_DIR, DEFAULT_CONFIG_PATH, ENV_PATH, ensure_user_default_config
...
HELP_TEXT: dict[str, str] = {
    ...
    "Inference backend": "Choose a local ONNX model folder or Roboflow API.",
    "Local ONNX folder": "Folder containing weights.onnx and its sidecar files such as class_names.txt and environment.json.",
    "Local model": "Resolved local model path used by the detection backend.",
    ...
    "Workers": "Number of parallel Roboflow tile requests. The local model runs sequentially in one model instance.",
...
def _app_icon() -> QIcon:
    candidates = [
        Path(__file__).resolve().with_name("stone.png"),
        APP_DIR / "stone.png",
    ]
...
class PathRow(QWidget):
    ...
    def choose(self) -> None:
        ...
        filters = {
            ...
            "model": "Model weights (*.onnx *.pt);;ONNX model (*.onnx);;PyTorch model (*.pt);;All files (*)",
            "pt": "Model weights (*.onnx *.pt);;ONNX model (*.onnx);;PyTorch model (*.pt);;All files (*)",
        }
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.load_config_button.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogOpenButton))
        self.run_button.clicked.connect(self._start)
        self.stop_button.clicked.connect(self._stop)
        self.save_config_button.clicked.connect(self._save_config_as)
        ...
        controls.addWidget(self.save_config_button)
        controls.addWidget(self.load_config_button)
    ...
    def _model_tab(self) -> QWidget:
        ...
        self.inference_backend.addItem("Local ONNX folder", "local_onnx_folder")
        self.inference_backend.addItem("Roboflow API", "roboflow")
        self.inference_backend.setItemData(
            0,
            "Run weights.onnx from a selected model folder.",
        ...
        self.local_onnx_folder = PathRow("dir", "Select local ONNX model folder", DEFAULT_LOCAL_ONNX_FOLDER)
        ...
        layout.addRow(self._help_label("Local ONNX folder"), self.local_onnx_folder)
    ...
    def _load_env_defaults(self) -> None:
        ...
        local_model = Path(values.get("LOCAL_YOLO_MODEL", os.environ.get("LOCAL_YOLO_MODEL", DEFAULT_LOCAL_MODEL)))
        if local_model.suffix.lower() == ".onnx":
            self.local_onnx_folder.set_path(local_model.parent)
        else:
            self.local_onnx_folder.set_path(values.get("LOCAL_ONNX_FOLDER", os.environ.get("LOCAL_ONNX_FOLDER", DEFAULT_LOCAL_ONNX_FOLDER)))
        backend = values.get("INFERENCE_BACKEND", os.environ.get("INFERENCE_BACKEND", "")).strip()
        if backend == "local_yolo":
            backend = "local_onnx_folder"
        backend_index = self.inference_backend.findData(backend)
        if backend_index >= 0:
            self.inference_backend.setCurrentIndex(backend_index)

    def _selected_local_model_path(self) -> Path:
        if str(self.inference_backend.currentData() or "") == "local_onnx_folder":
            return self.local_onnx_folder.path() / "weights.onnx"
        return self.local_onnx_folder.path() / "weights.onnx"
    ...
    def _write_config_file(self, path: Path, config: PipelineConfig) -> None:
    ...
    def _load_default_config(self) -> None:
        if not DEFAULT_CONFIG_PATH.exists():
            return
        try:
            data = self._read_config_file(DEFAULT_CONFIG_PATH)
            self._apply_config_data(data)
        except Exception as error:
            QMessageBox.warning(self, "Config load warning", f"Could not load default config:\n{error}")
    ...
    def _save_config_as(self) -> None:
        ...
        try:
            self._write_config_file(target, config)
            if target.resolve() != DEFAULT_CONFIG_PATH.resolve():
                self._write_config_file(DEFAULT_CONFIG_PATH, config)
        except Exception as error:
            QMessageBox.critical(self, "Save config failed", str(error))
            return
        self._append_log(f"Saved config: {target}")
        if target.resolve() != DEFAULT_CONFIG_PATH.resolve():
            self._append_log(f"Updated default config: {DEFAULT_CONFIG_PATH}")
    ...
    def _load_config_from_file(self) -> None:
        selected, _ = QFileDialog.getOpenFileName(
            self,
            "Load config",
            str(DEFAULT_CONFIG_PATH if DEFAULT_CONFIG_PATH.exists() else PROJECT_ROOT),
            "JSON config (*.json);;All files (*)",
        )
        if not selected:
            return
        ...
        try:
            data = self._read_config_file(source)
            self._apply_config_data(data)
            self._write_config_file(DEFAULT_CONFIG_PATH, self._build_config())
        except Exception as error:
            QMessageBox.critical(self, "Load config failed", str(error))
            return
        self._append_log(f"Loaded config: {source}")
        self._append_log(f"Updated default config: {DEFAULT_CONFIG_PATH}")

    def _apply_config_data(self, data: dict[str, object]) -> None:
        detection = data.get("detection") if isinstance(data.get("detection"), dict) else {}
        habitat = data.get("habitat") if isinstance(data.get("habitat"), dict) else {}

        run_detection = bool(data.get("run_detection", True))
        run_habitat = bool(data.get("run_habitat", True))
        if run_detection and run_habitat:
            self.mode.setCurrentText("Full pipeline")
        elif run_detection:
            self.mode.setCurrentText("Rock detection only")
        else:
            self.mode.setCurrentText("Habitat map only")

        self.output_root.set_path(str(data.get("output_root", DEFAULT_OUTPUT_ROOT)))
        self.run_name.setText(str(data.get("run_name", "")))

        self.orthomosaic.set_path(str(detection.get("image", DEFAULT_ORTHOMOSAIC)))
        backend_value = str(detection.get("inference_backend", "local_onnx_folder"))
        local_model_path = Path(str(detection.get("local_model", DEFAULT_LOCAL_MODEL)))
        if backend_value == "local_yolo":
            backend_value = "local_onnx_folder"
        local_model_source = str(detection.get("local_model_source", ""))
        if local_model_source == "onnx_folder":
            backend_value = "local_onnx_folder"
        backend_index = self.inference_backend.findData(backend_value)
        if backend_index >= 0:
            self.inference_backend.setCurrentIndex(backend_index)
        self.local_onnx_folder.set_path(
            str(detection.get("local_onnx_folder", local_model_path.parent if local_model_path.suffix.lower() == ".onnx" else DEFAULT_LOCAL_ONNX_FOLDER))
        )
        self.api_url.setText(str(detection.get("api_url", self.api_url.text())))
        self.api_key.setText(str(detection.get("api_key", self.api_key.text())))
        self.workspace.setText(str(detection.get("workspace", self.workspace.text())))
        self.workflow.setText(str(detection.get("workflow", self.workflow.text())))
        self.model_id.setText(str(detection.get("model_id", "")))

        self.tile_size.setValue(int(detection.get("tile_size", self.tile_size.value())))
        self.overlap.setValue(int(detection.get("overlap", self.overlap.value())))
        self.confidence.setValue(float(detection.get("confidence_threshold", self.confidence.value())))
        self.nms_iou.setValue(float(detection.get("nms_iou", self.nms_iou.value())))
        self.jpg_quality.setValue(int(detection.get("jpg_quality", self.jpg_quality.value())))
        self.workers.setValue(int(detection.get("workers", self.workers.value())))
        max_tiles = detection.get("max_tiles")
        self.max_tiles_enabled.setChecked(max_tiles is not None)
        if max_tiles is not None:
            self.max_tiles.setValue(int(max_tiles))
        self.overwrite.setChecked(bool(detection.get("overwrite", self.overwrite.isChecked())))
        self.green_filter.setChecked(bool(detection.get("green_filter", self.green_filter.isChecked())))
        self.green_threshold.setValue(float(detection.get("green_threshold", self.green_threshold.value())))
        self.green_margin.setValue(float(detection.get("green_margin", self.green_margin.value())))
        self.size_bins_enabled.setChecked(bool(detection.get("size_bins_enabled", self.size_bins_enabled.isChecked())))
        self.size_bins.setText(str(detection.get("size_bins", self.size_bins.text())))
        metric_index = self.size_metric.findData(str(detection.get("size_metric", "max_side_cm")))
        if metric_index >= 0:
            self.size_metric.setCurrentIndex(metric_index)
        cm_per_pixel = detection.get("cm_per_pixel")
        self.manual_cm_per_pixel.setChecked(cm_per_pixel is not None)
        if cm_per_pixel is not None:
            self.cm_per_pixel.setValue(float(cm_per_pixel))
        self.write_size_bin_files.setChecked(
            bool(detection.get("write_size_bin_shapefiles", self.write_size_bin_files.isChecked()))
        )
        self._refresh_habitat_size_bins()
        habitat_size_bin = str(detection.get("habitat_size_bin", ""))
        habitat_bin_index = self.habitat_size_bin.findData(habitat_size_bin)
        if habitat_bin_index >= 0:
            self.habitat_size_bin.setCurrentIndex(habitat_bin_index)

        self.vegetation.set_path(str(habitat.get("vegetation", "")))
        self.canopy.set_path(str(habitat.get("canopy", "")))
        self.existing_rocks.set_path(str(habitat.get("rocks", DEFAULT_OUTPUT_ROOT / "rocks.shp")))
        self.block_size.setText(str(habitat.get("block_size", self.block_size.text())))
        self.canopy_threshold.setValue(float(habitat.get("canopy_overlap_threshold", self.canopy_threshold.value())))
        self.score_scaling.setCurrentText(str(habitat.get("score_scaling", self.score_scaling.currentText())))
        self.vegetation_weight.setValue(float(habitat.get("vegetation_weight", self.vegetation_weight.value())))
        self.rock_weight.setValue(float(habitat.get("rock_weight", self.rock_weight.value())))
        self.rock_percentile.setValue(float(habitat.get("rock_percentile", self.rock_percentile.value())))
        rock_cap = habitat.get("rock_cap")
        self.rock_cap_enabled.setChecked(rock_cap is not None)
        if rock_cap is not None:
            self.rock_cap.setValue(float(rock_cap))
        self.rock_assignment.setCurrentText(str(habitat.get("rock_assignment", self.rock_assignment.currentText())))
        self.zone_breaks.setText(str(habitat.get("zone_breaks", self.zone_breaks.text())))
        self.zone_min_score.setValue(float(habitat.get("zone_min_score", self.zone_min_score.value())))
        self.zone_upscale.setValue(int(habitat.get("zone_upscale", self.zone_upscale.value())))
        self.zone_resampling.setCurrentText(str(habitat.get("zone_resampling", self.zone_resampling.currentText())))
        self.zone_connectivity.setCurrentText(str(habitat.get("zone_connectivity", self.zone_connectivity.currentText())))
        self.zone_simplify.setValue(float(habitat.get("zone_simplify", self.zone_simplify.value())))
        self.zone_smooth.setValue(float(habitat.get("zone_smooth", self.zone_smooth.value())))
        self.zone_min_area.setValue(float(habitat.get("zone_min_area", self.zone_min_area.value())))
        self.zone_explode.setChecked(bool(habitat.get("zone_explode", self.zone_explode.isChecked())))
    ...
    def _sync_inference_backend(self, _index: int | None = None) -> None:
        ...
        self.local_onnx_folder.setEnabled(backend == "local_onnx_folder")
    ...
    def _build_config(self) -> PipelineConfig:
        ...
        backend = str(self.inference_backend.currentData() or "roboflow")
        local_model = self._selected_local_model_path()

        detection = DetectionConfig(
            image=self.orthomosaic.path(),
            output=Path(self.output_root.edit.text()) / "rocks.shp",
            inference_backend="local_yolo" if backend == "local_onnx_folder" else backend,
            local_model_source=self._local_model_source(),
            local_onnx_folder=self.local_onnx_folder.path(),
            local_pt_model=DEFAULT_LOCAL_PT_MODEL,
    ...
    def _validate_config(self, config: PipelineConfig) -> bool:
        ...
        if (
            ...
            if config.detection.local_model_source == "onnx_folder":
                errors.append(f"Local ONNX weights not found: {config.detection.local_model}")
            elif config.detection.local_model_source == "pt_file":
                errors.append(f"Local .pt model not found: {config.detection.local_model}")
            else:
    ...
    def _format_result_summary(self, result: object) -> str:
        lines = [f"Outputs are in:\n{result.run_dir}"]
        ...
        if detection is not None:
            lines.extend(
                [
                    "",
                    "Rock detection:",
                    f"  Raw detections: {detection.raw_detection_count}",
                    f"  After NMS: {detection.final_detection_count}",
                    f"  Green-filter removed: {detection.green_filtered_count}",
                    f"  File: {detection.output_shp}",
                ]
            )
            if detection.size_bin_counts:
                lines.append("  Size bins:")
                lines.extend(f"    {label}: {count}" for label, count in detection.size_bin_counts.items())

        habitat = getattr(result, "habitat", None)
        if habitat is not None:
            lines.extend(
                [
                    "",
                    "Habitat map:",
                    f"  Grid cells: {habitat.cell_total}",
                    f"  Blocked by canopy: {habitat.blocked_total}",
                    f"  Non-zero score cells: {habitat.scored_total}",
                    f"  Zone polygons: {habitat.zone_total}",
                    f"  Zone file: {habitat.output_zones}",
                ]
            )
        return "\n".join(lines)
